...
router.post('/:id/learn-skill', async (req, res) => {
  try {
    ...
    const existing = await queryOne(
      'SELECT id, skill_level FROM char_skills WHERE character_id=? AND skill_id=?',
      [char.id, skillId]
    );

    if (existing && existing.skill_level >= 100) {
      return res.status(400).json({ error: '技能已達最高等級 (Lv.100)' });
    }
    ...
    await transaction(async (conn) => {
      ...
      if (existing) {
        ...
        // 未學習則新增並設為 Lv.1
        await conn.execute(
          'INSERT INTO char_skills (character_id, skill_id, skill_level) VALUES (?,?,1)',
          [char.id, skillId]
        );
      }
    });
...
async function getFullChar(charId, userId) {
  ...
  return {
    ...parseChar(char),
    // 將陣列轉換為 { 'fire1': 5, 'ice1': 2 } 格式以記錄技能等級
    learnedSkills: skills.reduce((acc, s) => {
      acc[s.skill_id] = s.skill_level || 1;
      return acc;
    }, {}),
