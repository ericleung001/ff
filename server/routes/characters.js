...
router.get('/', async (req, res) => {
  const chars = await query(
    `SELECT c.*, 
      GROUP_CONCAT(cs.skill_id) AS learned_skills
     FROM characters c
     LEFT JOIN char_skills cs ON cs.character_id = c.id
     WHERE c.user_id = ?
     GROUP BY c.id`,
    [req.user.userId]
  );
...
router.post('/', async (req, res) => {
  try {
    ...
    if (name.length < 1 || name.length > 20)
      return res.status(400).json({ error: '名字需1-20字' });

    // Max 3 characters per account
    const count = await queryOne(
      'SELECT COUNT(*) AS c FROM characters WHERE user_id=?', [req.user.userId]
    );
    ...
    const r = await execute(
      `INSERT INTO characters
        (user_id, name, job, max_hp, hp, max_mp, mp,
         stat_str, stat_int, stat_agi, stat_wis, stat_def)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        req.user.userId, name, job,
        base.maxHp, base.maxHp, base.maxMp, base.maxMp,
        base.stats.STR, base.stats.INT, base.stats.AGI,
        base.stats.WIS, base.stats.DEF,
      ]
    );
    ...
    for (const gear of starterGear) {
      await execute(
        `INSERT INTO char_equipment (character_id, slot, item_id, item_name, rarity, stat_bonus)
         VALUES (?,?,?,?,?,?)`,
...
// ── PATCH /api/characters/:id/story ──────────────────────
router.patch('/:id/story', async (req, res) => {
  const { storyIndex } = req.body;
  await execute(
    'UPDATE characters SET story_index=? WHERE id=? AND user_id=?',
    [storyIndex, req.params.id, req.user.userId]
  );
  res.json({ ok: true });
});
...
router.post('/:id/learn-skill', async (req, res) => {
  try {
    ...
    const exists = await queryOne(
      'SELECT id FROM char_skills WHERE character_id=? AND skill_id=?',
      [char.id, skillId]
    );
    if (exists) return res.status(400).json({ error: '已學習此技能' });

    await transaction(async (conn) => {
      await conn.execute(
        'UPDATE characters SET sp=sp-? WHERE id=?', [cost, char.id]
      );
      await conn.execute(
        'INSERT INTO char_skills (character_id, skill_id) VALUES (?,?)',
        [char.id, skillId]
      );
    });
...
router.post('/:id/equip', async (req, res) => {
  try {
    ...
    await transaction(async (conn) => {
      // Unequip old item back to inventory if exists
      const old = await queryOne(
        'SELECT * FROM char_equipment WHERE character_id=? AND slot=?', [char.id, slot]
      );
      if (old) {
        await conn.execute(
          ...
          [char.id, old.item_id, old.item_name, slot, old.rarity, old.stat_bonus]
        );
        await conn.execute(
          'DELETE FROM char_equipment WHERE character_id=? AND slot=?', [char.id, slot]
        );
      }
      // Equip new item
      await conn.execute(
        `INSERT INTO char_equipment (character_id, slot, item_id, item_name, rarity, stat_bonus)
         VALUES (?,?,?,?,?,?)`,
        [char.id, slot, invItem.item_id, invItem.item_name, invItem.rarity, invItem.stat_bonus]
      );
      // Remove from inventory
      if (invItem.quantity > 1) {
        await conn.execute(
          'UPDATE inventory SET quantity=quantity-1 WHERE id=?', [invItem.id]
        );
      } else {
        await conn.execute('DELETE FROM inventory WHERE id=?', [invItem.id]);
      }
    });
...
// ── POST /api/characters/:id/solo-reward ─────────────────
// Called after solo combat victory — persists XP, gold, loot, handles level-up
router.post('/:id/solo-reward', async (req, res) => {
  try {
    ...
    // Loot → determine rarity & type from name (material by default)
    const LOOT_META = {
      '小型符文石':  { type:'material', rarity:'common'  },
      '骨質碎片':    { type:'material', rarity:'common'  },
      '暗影匕首':    { type:'weapon',   rarity:'rare'    },
      '惡魔之核':    { type:'material', rarity:'rare'    },
      '混沌石板':    { type:'material', rarity:'epic'    },
      '守門者之鑰':  { type:'accessory',rarity:'legend'  },
    };
    const meta = LOOT_META[loot] || { type:'material', rarity:'common' };

    // Compute new XP / level
    let newXp    = (char.xp || 0) + (xp || 0);
    let newGold  = (char.gold || 0) + (gold || 0);
    let newLevel = char.level;
    let newXpNext= char.xp_next;
    let newMaxHp = char.max_hp;
    let newMaxMp = char.max_mp;
    let newSp    = char.sp;
    let newStr   = char.stat_str, newInt = char.stat_int;
    let newAgi   = char.stat_agi, newWis = char.stat_wis, newDef = char.stat_def;
    let leveledUp = false;
    ...
    await transaction(async (conn) => {
      ...
      await conn.execute(
        ...
        [
          ...
          newMaxHp, leveledUp ? newMaxHp : char.hp,
          newMaxMp,
          newSp,
          newStr, newInt, newAgi, newWis, newDef,
          char.id,
      ...
      if (loot) {
        ...
        await conn.execute(
          ...
          [char.id, lootId, loot, meta.type, meta.rarity, '{}']
        );
      }
    });
    ...
    console.error('[solo-reward]', err);
    res.status(500).json({ error: '伺服器錯誤' });
  }
});

// ── POST /api/characters/:id/save-hp ─────────────────────
router.post('/:id/save-hp', async (req, res) => {
  try {
    const { hp, mp } = req.body;
    await execute(
      'UPDATE characters SET hp=?, mp=? WHERE id=? AND user_id=?',
      [hp ?? 0, mp ?? 0, req.params.id, req.user.userId]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: '伺服器錯誤' });
...
async function getFullChar(charId, userId) {
  const char = await queryOne(
    'SELECT * FROM characters WHERE id=? AND user_id=?', [charId, userId]
  );
  if (!char) return null;

  const skills  = await query('SELECT skill_id FROM char_skills WHERE character_id=?', [charId]);
  ...
  return {
    ...
    equipment: equip.map(e => ({
      slot: e.slot, itemId: e.item_id, name: e.item_name,
      rarity: e.rarity, bonus: JSON.parse(e.stat_bonus || '{}'),
    })),
    inventory: inv.map(i => ({
      id: i.id, itemId: i.item_id, name: i.item_name,
      type: i.item_type, rarity: i.rarity,
      bonus: JSON.parse(i.stat_bonus || '{}'), quantity: i.quantity,
    })),
...
function getStarterGear(job) {
  const map = {
    mage:    [{ slot:'weapon', itemId:'staff_wood',    name:'木製法杖', rarity:'common', bonus:{INT:2} },
              { slot:'armor',  itemId:'robe_cloth',    name:'布質長袍', rarity:'common', bonus:{MP:10} }],
    warrior: [{ slot:'weapon', itemId:'sword_iron',    name:'鏽鐵劍',   rarity:'common', bonus:{STR:2} },
              { slot:'armor',  itemId:'armor_leather', name:'皮甲',     rarity:'common', bonus:{DEF:3} }],
    rogue:   [{ slot:'weapon', itemId:'dagger_rusty',  name:'鏽匕首',   rarity:'common', bonus:{AGI:2} },
              { slot:'armor',  itemId:'cloak_shadow',  name:'暗影斗篷', rarity:'common', bonus:{AGI:1} }],
    priest:  [{ slot:'weapon', itemId:'scepter_holy',  name:'聖光法杖', rarity:'common', bonus:{WIS:2} },
              { slot:'armor',  itemId:'robe_holy',     name:'聖職法衣', rarity:'common', bonus:{MP:15} }],
  };
  return map[job] || [];
}
